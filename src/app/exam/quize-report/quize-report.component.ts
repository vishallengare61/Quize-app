...
data(quizeID: any){
  ...
  // console.log('data getting id', this.quizePoolId);
  // this.getresultData(this.quizePoolId);
  return new Promise((resolve) => {
...
clearCharts() {
  ...
  const chartElements2 = document.querySelectorAll('.chart2');
  const chartElements3 = document.querySelectorAll('.chart3');
  ...
  chartElements2.forEach((element) => {
    const chart = ApexCharts.exec(element.id, 'destroy');
    if (chart) {
      chart.destroy();
    }
  });
  chartElements3.forEach((element) => {
    const chart = ApexCharts.exec(element.id, 'destroy');
    if (chart) {
      chart.destroy();
    }
